...
extend(qrcode.prototype,{
    ...
    createSVG (qrCodeAlg){
        ...
        let scale = this.options.height / this.options.width;

        // create svg
        let svg = document.createElementNS("http://www.w3.org/2000/svg", "svg");
        svg.setAttribute("width", this.options.width);
        svg.setAttribute("height", this.options.height);
        svg.setAttribute("viewbox", `0 0 ${count * 10} ${count * 10 * scale}`);
        ...
        for (let row = 0; row < count; row++) {
            for (let col = 0; col < count; col++) {
                ...
                path.setAttribute('d', `M ${x},${y} L ${dx},${y} L ${dx},${dy} L ${x},${dy} Z`);
                path.setAttribute('stroke-width', '0.5');

                if(qrCodeAlg.modules[row][ col]){
                    path.setAttribute('fill', foreground);
                    path.setAttribute('stroke', foreground);
                }else{
                    path.setAttribute('fill', this.options.background);
                    path.setAttribute('stroke', this.options.background);
                }
        ...
        if(this.options.image){
            ...
            img.setAttributeNS("http://www.w3.org/1999/xlink","href",this.options.image);
            img.setAttribute('x', ((this.options.width - this.options.imageWidth)/2).toFixed(2));
            img.setAttribute('y', ((this.options.height - this.options.imageHeight)/2).toFixed(2));
            img.setAttribute('width', this.options.imageWidth);
            img.setAttribute('height', this.options.imageHeight);
            svg.appendChild(img);
        }
